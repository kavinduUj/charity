...
class EventDetailsUseCase @Inject constructor(
    private val eventRepo: EventRepo
) {
    operator fun invoke(tok: String, body:String): Flow<Resource<EventDetailsResponse>> =
        flow {
            try {
                emit(Resource.Loading())
                val loginData = eventRepo.eventDetails(tok, body)
